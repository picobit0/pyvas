...
# Этап 5
## Список изменений:
Добавлена программа, применяющая операцию sgn() ко всем элементам массива размера 6.

## Демонстрация работы:
Исходный код тестового примера 1:
```
: запись в память массива длины 6 [-1, -2, -3, 3, 2, 1]
LD 1
LD 2
LD 3
LD -3
...
: применение операции sgn() ко всем элементам массива
LD 0
LD 0
LD 0
LD 0
...
: -1, -1, -1, 1, 1, 1
```
Результат выполнения тестового примера 1:
```
-1,-1,-1,1,1,1
```

Исходный код тестового примера 2:
```
: запись в память массива длины 6 [99, 0, -99, -99, 0, 99]
LD 99
LD 0
LD -99
LD -99
...
: 1, 0, -1, -1, 0, 1
```
Результат выполнения тестового примера 2:
```
1,0,-1,-1,0,1
```

Исходный код тестового примера 3:
```
: запись в память массива длины 6 [0, 0, 0, 0, 0, 37]
LD 37
WR 5
...
: 0, 0, 0, 0, 0, 1
```
Результат выполнения тестового примера 3:
```
0,0,0,0,0,1
```
...
# Этап 4
## Список изменений:
Добавлена поддержка команды `SG` в интерпретаторе

`SG <адрес>` - снимает значение со стека, записывает в адрес 1, если оно положительное, -1 если отрицательное, иначе 0.

## Демонстрация работы:
Исходный код теста операции `SG`:
```
LD 1
SG 0 : запись результата sgn(1) по адресу 0

LD -10
SG 1 : запись результата sgn(-10) по адресу 1

LD 0
SG 2 : запись результата sgn(0) по адресу 2

LD -1
SG 3 : запись результата sgn(-1) по адресу 3

: ожидаемый результат - 1, -1, 0, -1
```
Дамп памяти интерпретации программы (отрезок 0-3)
```
1,-1,0,-1
```
